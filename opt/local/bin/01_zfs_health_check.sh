...
if [ ${problems} -eq 0 ]; then
  ...
  for volume in ${zfsVolumes}
   do
    ...
    ### SmartOs solution. This needs dateutils installed
    scrubRawDate=$(/sbin/zpool status zones | grep scrub | awk '{print $13" "$12" "$15" "$14}')

    # if [ $(($currentDate - $scrubDate)) -ge $scrubExpire ]; then
     if [ $(ddiff -i '%d %b %Y %H:%M:%S' "${scrubRawDate}" now -f '%S') -ge $scrubExpire ]; then
...
if [ "$problems" -ne 0 ]; then
  printf '%s\n' "$emailSubject" "" "`/sbin/zpool list`" "" "`/sbin/zpool status`" | /usr/bin/mail -s "$emailSubject" root@smartos.aerrow.local
  logger $emailSubject
fi
